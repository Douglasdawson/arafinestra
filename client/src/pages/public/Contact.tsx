...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        setSuccess(true);
        setForm({ nombre: "", email: "", telefono: "", mensaje: "", tipoCliente: "particular" });
      } else {
        setError(true);
      }
    } catch {
    ...
    <>
      ...
      <section className="py-16 bg-gradient-to-br from-slate-800 to-blue-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h1 className="text-4xl sm:text-5xl font-bold text-white">{t("contact.title")}</h1>
        </div>
      </section>
      ...
      <section className="py-12 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Form */}
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">{t("contact.form_title")}</h2>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">{t("contact.client_type")}</label>
                  <select
                    name="tipoCliente"
                    value={form.tipoCliente}
                    onChange={handleChange}
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 bg-white"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={handleChange}
                    rows={5}
                    className="w-full px-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-500 focus:border-sky-500 resize-none"
                  />
                </div>
                <button
                  type="submit"
                  disabled={sending}
                  className="w-full px-6 py-3 bg-sky-500 text-white font-semibold rounded-lg hover:bg-sky-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {/* Contact info */}
            <div>
              <h2 className="text-2xl font-bold text-slate-800 mb-6">{t("contact.info_title")}</h2>
              <div className="space-y-6">
                {/* Phone */}
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 flex-shrink-0 flex items-center justify-center rounded-full bg-sky-100 text-sky-600">
                  ...
                  <div>
                    ...
                    <a href="tel:+34XXXXXXXXX" className="text-sm text-sky-600 hover:text-sky-700">
                      +34 XXX XXX XXX
                    </a>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <a href="mailto:info@arafinestra.com" className="text-sm text-sky-600 hover:text-sky-700">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{t("contact.address")}</p>
                    <p className="text-sm text-slate-600">Girona, Catalunya</p>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-slate-800">{t("contact.hours")}</p>
                    <p className="text-sm text-slate-600">{t("contact.hours_detail")}</p>
                  </div>
                </div>
